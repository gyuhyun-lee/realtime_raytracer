...
void VkApp::destroyAllVulkanResources()
{
    ...
    for (uint32_t i = 0; 
        i < m_framebuffers.size(); 
        ++i)
    {
        vkDestroyFramebuffer(m_device, m_framebuffers[i], 0);
    }

    vkDestroyFramebuffer(m_device, m_scanlineFramebuffer, 0);


    // Destroy all textures with:  
    for (auto &t : m_objText)
    {
        t.destroy(m_device); 
    }
    // Destroy all buffers with:   
    for (auto &ob:m_objData) 
    {
        ob.destroy(m_device);
    }
    // TODO(gyuhyun) Is this the right way to do this?
    m_objDesc.resize(0);
    ...
    vkDestroyPipeline(m_device, m_scanlinePipeline, 0);
    vkDestroyPipelineLayout(m_device, m_scanlinePipelineLayout, 0);

    vkDestroyPipeline(m_device, m_postPipeline, 0);
    vkDestroyPipelineLayout(m_device, m_postPipelineLayout, 0);

    vkDestroyPipelineLayout(m_device, m_rtPipelineLayout, nullptr);
    vkDestroyPipeline(m_device, m_rtPipeline, nullptr);

    vkDestroyRenderPass(m_device, m_scanlineRenderPass, 0);
    vkDestroyRenderPass(m_device, m_postRenderPass, 0);
...
void VkApp::createInstance(bool doApiDump)
{
    ...
    for (uint32_t i = 0;
        i < countGLFWextensions;
        ++i)
    {
        printf("%s", *(reqGLFWextensions + i));
    ...
    for (uint32_t i = 0;
        ...
        ++i)
    {
        printf("%uth available instance extension : %s\n", i, availableExtensions[i].extensionName);
    }
        ...
        if (GPUproperties.deviceType == VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU)
        {
            ...
            for (const char* requiredExtension : reqDeviceExtensions)
            {
                ...
                if (!found)
                {
                    ...
                    printf("%s extension not found\n", requiredExtension);
                    break;
                }
                else
                {
                    printf("%s extension found\n", requiredExtension);
                }
            }
...
void VkApp::chooseQueueIndex()
{
    ...
    uint32_t index = 0;
    for (const VkQueueFamilyProperties& property : queueProperties)
    {
...
void VkApp::getSurface()
{
    ...
    CHECKVKRESULT(glfwCreateWindowSurface(m_instance, app->GLFW_window, nullptr, &m_surface));
    CHECKVKRESULT(vkGetPhysicalDeviceSurfaceSupportKHR(m_physicalDevice, m_graphicsQueueIndex,
...
void VkApp::createCommandPool()
{
    ...
    CHECKVKRESULT(vkCreateCommandPool(m_device, &poolCreateInfo, nullptr, &m_cmdPool));
    ...
    CHECKVKRESULT(vkAllocateCommandBuffers(m_device, &allocateInfo, &m_commandBuffer));
...
void VkApp::createSwapchain()
{
    ...
    // Check Whether The Present Mode We Required Is Available
    ...
    assert(foundDesiredPresentMode);
    ...
    // Do the two step process to retrieve a list of surface formats
    uint32_t formatCount;
    vkGetPhysicalDeviceSurfaceFormatsKHR(m_physicalDevice, m_surface, &formatCount, 0);
    ...
    // @@ Document your list of formats/color-spaces, and your
    // particular choice.
    ...
    CHECKVKRESULT(vkGetSwapchainImagesKHR(m_device, m_swapchain, &swapchainImageCount, 0));
    m_swapchainImages.resize(swapchainImageCount);
    CHECKVKRESULT(vkGetSwapchainImagesKHR(m_device, m_swapchain, &swapchainImageCount, m_swapchainImages.data()));

    assert(swapchainImageCount == imageCount);
    ...
    for (uint i = 0; i < m_imageCount; i++) {
        ...
        CHECKVKRESULT(vkCreateImageView(m_device, &createInfo, nullptr, &m_imageViews[i]));
    }
    ...
    CHECKVKRESULT(vkCreateFence(m_device, &fenceCreateInfo, nullptr, &m_waitFence));

    VkSemaphoreCreateInfo semCreateInfo = { VK_STRUCTURE_TYPE_SEMAPHORE_CREATE_INFO };
    CHECKVKRESULT(vkCreateSemaphore(m_device, &semCreateInfo, nullptr, &m_readSemaphore));
    CHECKVKRESULT(vkCreateSemaphore(m_device, &semCreateInfo, nullptr, &m_writtenSemaphore));
...
void VkApp::destroySwapchainImageViews()
{
    for (uint32_t swapchainImageIndex = 0;
        swapchainImageIndex < m_imageCount;
        ++swapchainImageIndex)
    {
        vkDestroyImageView(m_device, m_imageViews[swapchainImageIndex], 0);
    }
}
    ...
    VkMemoryPropertyFlags properties, uint mipLevels)
{
    ...
    CHECKVKRESULT(vkCreateImage(m_device, &imageInfo, nullptr, &myImage.image));
    ...
    CHECKVKRESULT(vkAllocateMemory(m_device, &allocInfo, nullptr, &myImage.memory));
    ...
    VkImageAspectFlagBits aspect)
{
    ...
    CHECKVKRESULT(vkCreateImageView(m_device, &viewInfo, nullptr, &imageView));
...
void VkApp::createPostFrameBuffers()
{
    ...
    //createInfo.pPushConstantRanges    = &pushConstantRanges;
        ...
        CHECKVKRESULT(vkCreateFramebuffer(m_device, &_ci, nullptr, &m_framebuffers[i]));
    }
...
void VkApp::createPostPipeline()
{
    ...
    CHECKVKRESULT(vkCreateGraphicsPipelines(m_device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr,
        &m_postPipeline));

    // The pipeline has fully compiled copies of the shaders, so these
    // intermediate (SPV) versions can be destroyed.
    vkDestroyShaderModule(m_device, vertShaderModule, 0);
    vkDestroyShaderModule(m_device, fragShaderModule, 0);
...
void VkApp::postProcess()
{
    ...
    {   // extra indent for renderpass commands
        ...
        vkCmdDraw(m_commandBuffer, 3, 2, 0, 0);
